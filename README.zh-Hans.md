...
行棋阶段支持普通移动、同色或异色捉子、推动、拉动、议和交换，以及带目标的认负。
...
记谱必须基于当前游戏解析，因为阶段、盘上身份、相对移动和目标化认负都依赖这份局面。
...
`GameConfig` 与快照协议支持最大 16×16 的矩形棋盘，也允许描述不必能从标准开局到达、但内部规则一致的局面。校验仍会检查棋池所属方、布阵半场、交替棋池大小、首领棋子数量和声明结果。可接受格式见[文本记谱法](docs/notation.zh-Hans.md)。
